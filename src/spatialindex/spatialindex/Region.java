...
public class Region implements IShape
{
	...
	public boolean equals(Object o)
	{
		if (o instanceof Region)
		{
			...
			{
				if (m_pLow[cIndex] < r.m_pLow[cIndex] - SpatialIndex.EPSILON || m_pLow[cIndex] > r.m_pLow[cIndex] + SpatialIndex.EPSILON ||
						m_pHigh[cIndex] < r.m_pHigh[cIndex] - SpatialIndex.EPSILON || m_pHigh[cIndex] > r.m_pHigh[cIndex] + SpatialIndex.EPSILON)
						return false;
			}
			return true;
		}
	...
	public boolean touches(final Point p)
	{
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
		{
			if ((m_pLow[cIndex] > p.m_pCoords[cIndex] - SpatialIndex.EPSILON && m_pLow[cIndex] < p.m_pCoords[cIndex] + SpatialIndex.EPSILON) ||
					(m_pHigh[cIndex] > p.m_pCoords[cIndex] - SpatialIndex.EPSILON && m_pHigh[cIndex] < p.m_pCoords[cIndex] + SpatialIndex.EPSILON))
					return true;
		}
		return false;
	}
